...
export class DashboardComponent implements OnInit {
  ...
  constructor(
    ...
    window.addEventListener("offline", () => {
      this.appService.errorToastr = this.toastrService.error(
        "Connection Lost",
        "Connection",
        {
          tapToDismiss: false,
      ...
      setTimeout(() => {
        this.initializeNECWebSocketConnection(this.appService.errorToastr);
      }, 5000);
    });
  }
    ...
    this.stompClient.connect(
      ...
      function (frame) {
        ...
        that.stompClient.subscribe("/realtime/nec", (message) => {
          ...
          localStorage.setItem("tempNECStatus", JSON.stringify(JSON.parse(message.body)));
          if (message.body) {
